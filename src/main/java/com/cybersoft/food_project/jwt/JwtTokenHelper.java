...
public class JwtTokenHelper {
    ...
    public String generateToken(String data, String type, long expiredDate) {
        ...
        return Jwts.builder()
                .setSubject(json)           //lưu trữ dữ liệu vào token kiểu String (json)
                .setIssuedAt(now)                 //tgian tạo token
                .setExpiration(dateExpired)         //tgian hết hạn của token
                .signWith(secretKey, SignatureAlgorithm.HS256)   //thuật toán mã hóa và secret key
                .compact();                         //trả ra Token đã dc mã hóa
    }
